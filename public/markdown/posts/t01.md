...
- **Fetching a file that lives in the src/ directory:**
    ...
    - **WHY:** Fetching occurs at runtime, which means your code has already undergone bundling by that point, so there's no way you can ever reference anything in there. But if you are still interested, my tests returned index.html to me no matter what I tried to fetch (even if I made the file up).
