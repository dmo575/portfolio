## Configuring your Spring Boot app API (MVC, REST, HTTP)
...
I will show you now what you could do for each of those cases, and then I will show you what I think is the proper way to do it, which works for both of these cases.

**Gone wrong: REST**
In this case we just use ResponseEntity. That's it, just have your method return a ResponseEntity. It is obviously still a REST API you just have to be a bit more manual with the endpoint's definition.
You may be thinking "But I have @ResponseBody so I can't really return the whole HTTP, how does that work?"
Even if you are using @RESTController for the class or a @ResponseBody for the method, if you return a ResponseEntity, Spring will scrap the @ResponseBody annotation and will think that you want to return the whole HTTP response.
...
public class MyClass {
	...
	@RequestMapping(path="/", method=RequestMethod.GET)
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<Animal> index() {
		try {
			Animal a = new Animal("Cow");
			. . .
...
```
Take a look at the controller annotation; it is a @RESTController annotation, which adds the @ResponseBody to all the class' endpoints. But because we are returning a ResponseEntity object, Spring doesn't ignores that and assumes you will return the whole HTTP message.
Now, this approach doesnt leave us with the option of sending a message in the response body so its not an optimal solution. We will take a look at how to better do this later.


**Gone wrong: MVC**

**Gone wrong: The correct way to handle it**
...
The last part is how to access the request HTTP data.

- variable {name}
- values /?question=how-much&answer=two
- HTTP body


### Trivia
Here are some curiosities over the material we covered. Many of them we mentioned during the post but some we did not.
- How is Spring able to serialize objects into JSON automatically for us? -> It uses the Jackson library for that.
- ...
...
We will start by taking a look at Controllers.

**Controllers**
Controllers in Spring are a type of bean that is to hold endpoints. So all the endpoints we create have to live as methods inside a Controller bean.
You can use the class level annotation @Controller to assign a class as a controller.
...
**Creating endpoints inside a Controller**
When creating and endpoint inside a Controller, you use methods.
